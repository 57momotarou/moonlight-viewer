(() => {
  ...
  const els = {
    loginScreen: $("#loginScreen"), viewerScreen: $("#viewerScreen"),
    connectionSetup: $("#connectionSetup"), setupUrl: $("#setupUrl"), setupKey: $("#setupKey"),
    saveConnectionBtn: $("#saveConnectionBtn"), toggleSetupBtn: $("#toggleSetupBtn"),
    loginForm: $("#loginForm"), emailInput: $("#emailInput"), passwordInput: $("#passwordInput"),
    loginStatus: $("#loginStatus"), refreshBtn: $("#refreshBtn"), logoutBtn: $("#logoutBtn"),
    loadingOverlay: $("#loadingOverlay"), toast: $("#toast"), cloudMeta: $("#cloudMeta"),
    overviewCards: $("#overviewCards"), overviewAchievements: $("#overviewAchievements"),
    salesList: $("#salesList"), deliveryList: $("#deliveryList"), buyersList: $("#buyersList"),
    productsList: $("#productsList"), inventoryLatest: $("#inventoryLatest"),
    inventoryHistory: $("#inventoryHistory"), bonusSummary: $("#bonusSummary"),
    bonusCurrent: $("#bonusCurrent"), payoutHistory: $("#payoutHistory"), employeeList: $("#employeeList"),
    moreSheet: $("#moreSheet"), closeMoreBtn: $("#closeMoreBtn")
  };
  ...
  function connection() {
    ...
    let local = {};
    try { local = JSON.parse(localStorage.getItem(CONFIG_STORAGE) || "{}"); } catch {}
    return {
  ...
  function session() {
    try { return JSON.parse(localStorage.getItem(SESSION_STORAGE) || "null"); } catch { return null; }
  }
  ...
  function showLoading(on) { els.loadingOverlay.classList.toggle("hidden", !on); }
  function toast(msg) {
  ...
  async function authFetch(path, options = {}, allowRefresh = true) {
    ...
    const res = await fetch(c.url + path, { ...options, headers });
  ...
  function employeeForRecord(r) {
    return (state.employees || []).find(e => String(e.id) === String(r.employeeId)) ||
      (state.employees || []).find(e => String(e.name||"") === String(r.employeeName||"")) || null;
  }
  ...
  function employeeSalesMap() {
    ...
    for (const r of state.dailySales || []) {
      ...
      const a = recordAmounts(r), t = map.get(String(id));
  ...
  function buyerProfiles() {
    return (state.buyers || []).map(b => ({
      id:String(b.id||""), name:String(b.name||b.displayName||"").trim(),
  ...
  function buyerSummary() {
    ...
    for(const r of state.dailySales||[]) {
      const x=ensure(r.buyerName, r.invoiceAt||r.saleDate); if(!x) continue;
      x.count++; x.store++; x.food+=num(r.foodQty); x.drink+=num(r.drinkQty); x.joint+=num(r.jointQty); x.other+=num(r.otherAmount);
    }
    for(const d of state.deliveryOrders||[]) {
      const x=ensure(d.buyerName, d.recordedAt); if(!x) continue;
      x.count++; x.delivery++; x.food+=num(d.foodQty); x.drink+=num(d.drinkQty); x.joint+=num(d.jointQty);
    }
  ...
  function renderOverview() {
    ...
    ].map(([l,v])=>`<div class="stat"><span class="label">${l}</span><strong>${v}</strong></div>`).join("");

    const salesMap=employeeSalesMap();
    els.overviewAchievements.innerHTML=sortedEmployees().map(e=>{
  ...
  function renderSales() {
    ...
    els.salesList.innerHTML=rows.map(r=>{
      const a=recordAmounts(r);
      const emp=employeeForRecord(r);
      return `<article class="item-card">
        <div class="item-top"><div><div class="item-title">${esc(emp?.name||r.employeeName||"-")}</div><div class="item-meta">${invoiceAt(r.invoiceAt||r.saleDate)} / 購入者 ${esc(r.buyerName||"-")}</div></div><div class="money">${money(a.total)}</div></div>
        <div class="pills"><span class="pill">食 ${qty(r.foodQty)} × ${money(r.foodUnitPrice||0)}</span><span class="pill">飲 ${qty(r.drinkQty)} × ${money(r.drinkUnitPrice||0)}</span><span class="pill">J ${qty(r.jointQty)} × ${money(r.jointUnitPrice||0)}</span>${a.other?`<span class="pill">その他 ${money(a.other)}</span>`:""}</div>
      </article>`;
    ...
    els.deliveryList.innerHTML=deliveries.map(d=>{
      const items=Array.isArray(d.items)&&d.items.length ? d.items.map(i=>`${esc(i.name||categoryLabel(i.category))} ×${Math.round(num(i.count))}`).join(" / ") : `食 ${qty(d.foodQty)} / 飲 ${qty(d.drinkQty)} / J ${qty(d.jointQty)}`;
      return `<article class="item-card"><div class="item-top"><div><div class="item-title">${esc(d.orderNo||"デリバリー")}</div><div class="item-meta">${invoiceAt(d.recordedAt)} / 注文者 ${esc(d.buyerName||"-")}</div></div></div><div class="pills"><span class="pill">${items}</span></div></article>`;
    }).join("") || empty("デリバリー履歴がありません");
  }

  function renderBuyers() {
    const rows=buyerSummary();
    els.buyersList.innerHTML=rows.map(b=>`<article class="item-card">
      ...
      <div class="key-values"><span>食べ物</span><strong>${qty(b.food)}</strong><span>飲み物</span><strong>${qty(b.drink)}</strong><span>ジョイント</span><strong>${qty(b.joint)}</strong><span>その他売上</span><strong>${money(b.other)}</strong></div>
    </article>`).join("") || empty("購入者がいません");
  }

  function renderProducts() {
    const rows=(state.productCatalog||[]).filter(p=>p.active!==false).sort((a,b)=>String(a.name||"").localeCompare(String(b.name||""),"ja"));
    els.productsList.innerHTML=rows.map(p=>`<article class="product-card">
    ...
    </article>`).join("") || empty("商品が登録されていません");
  }
  ...
  function renderSnapshot(s, latest=false) {
    ...
    const products=s.products.map(p=>`<div class="inventory-product"><strong>${esc(p.name||"-")}</strong><br><span class="muted">${esc(categoryLabel(p.category))}</span> ${qty(p.count)}</div>`).join("");
  ...
  function currentBonusEntries() {
    ...
    for(const r of state.dailySales||[]) {
      ...
      const e=employeeForRecord(r); if(!e) continue;
  ...
  function renderBonus() {
    ...
    els.bonusCurrent.innerHTML=entries.map(x=>`<article class="item-card">
      <div class="item-top"><div><div class="item-title">${esc(x.e.name)}</div><div class="item-meta">${esc(x.e.role||"-")}${x.manager?" / 余り支給":""}</div></div><div class="money">${money(x.amount)}</div></div>
      <div class="key-values"><span>期間売上</span><strong>${money(x.sales)}</strong><span>役職固定</span><strong>${money(x.roleBonus)}</strong><span>売上分</span><strong>${money(x.salesBonus)}</strong><span>コイン分</span><strong>${money(x.coinBonus)}</strong></div>
    </article>`).join("") || empty("対象従業員がいません");

    const hist=[...(state.payoutHistory||[])].sort((a,b)=>String(b.payoutDate||"").localeCompare(String(a.payoutDate||"")));
    els.payoutHistory.innerHTML=hist.map(h=>`<article class="item-card">
      <div class="item-top"><div><div class="item-title">${dateOnly(h.payoutDate)}</div><div class="item-meta">${dateOnly(h.periodStart||h.bonusStartDate)}〜${dateOnly(h.periodEnd||h.bonusEndDate)}</div></div><div class="money">${money(h.totalAmount||h.actualTotal||h.total)}</div></div>
      <div class="pills">${(h.entries||h.payouts||[]).map(x=>`<span class="pill">${esc(x.employeeName||x.name||"-")} ${money(x.amount)}</span>`).join("")}</div>
    </article>`).join("") || empty("支給履歴がありません");
  }

  function renderEmployees() {
    els.employeeList.innerHTML=sortedEmployees().map(e=>`<article class="item-card">
    ...
    </article>`).join("") || empty("従業員がいません");
  }

  function renderAll() {
    renderOverview(); renderSales(); renderBuyers(); renderProducts(); renderInventory(); renderBonus(); renderEmployees();
  }
  ...
  async function enterViewer() {
    ...
    try {
      ...
      refreshTimer=setInterval(()=>{ if(document.visibilityState==="visible") loadCloud().catch(()=>{}); },60000);
  ...
  els.saveConnectionBtn.addEventListener("click",()=>{
    const url=els.setupUrl.value.trim().replace(/\/+$/,""), key=els.setupKey.value.trim();
    if(!/^https:\/\/.+\.supabase\.co$/i.test(url) || !key.startsWith("sb_publishable_")) {
      els.loginStatus.textContent="URLまたはPublishable keyを確認してください。"; return;
    }
  ...
  els.loginForm.addEventListener("submit",async(e)=>{
    e.preventDefault(); els.loginStatus.textContent="ログイン中…";
    try {
      await login(els.emailInput.value.trim(),els.passwordInput.value);
      els.passwordInput.value="";
      await enterViewer();
    } catch(err) { els.loginStatus.textContent=err.message||"ログインに失敗しました"; }
  });

  els.refreshBtn.addEventListener("click",async()=>{showLoading(true);try{await loadCloud();toast("最新データに更新しました");}catch(e){toast(e.message);}finally{showLoading(false);}});
  els.logoutBtn.addEventListener("click",()=>{clearSession();location.reload();});

  $$(".nav-btn").forEach(btn=>btn.addEventListener("click",()=>{
    const name=btn.dataset.pageTarget;
    if(name==="more"){els.moreSheet.classList.toggle("hidden");els.moreSheet.setAttribute("aria-hidden",String(els.moreSheet.classList.contains("hidden")));return;}
    showPage(name);
  }));
  ...
  $$("[data-sales-tab]").forEach(btn=>btn.addEventListener("click",()=>{
  ...
  async function init() {
    ...
    if(session()?.access_token) {
      try { await enterViewer(); return; }
      catch(e){ clearSession(); els.loginStatus.textContent="再ログインしてください。"; }
    }
  }
